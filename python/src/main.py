...
class DL2Editor:
    ...
    async def load_keys_for_file(self, file_path):
        with open(file_path, 'r') as f:
            return json.load()


    async def process_keys(self, file_path, keys, key_dict):
    ...
    async def load_json_file_and_process(self, file_path, key_dict):
        start_time = time.time()
        keys = await self.load_keys_for_file(file_path)
        await asyncio.wait_for(self.process_keys(file_path, keys, key_dict), timeout=0.01)
        end_time = time.time()
    ...
    async def load_json_files(self):
        ...
        for json_file in self.json_files:
            task = asyncio.create_task(self.load_json_file_and_process(json_file, key_dict))
            tasks.append(task)
    ...
    def open_save(self):
        ...
        isValid = self.userInterface.open_save(file_path, backup_created)
        if isValid:
            try:
                    loop = asyncio.get_event_loop()
                    key_dict = loop.run_until_complete(self.load_json_files())
                ...
                self.userInterface.showError("Error", str(e))

    def create_backup(self, file_path):
        backup_dir = os.path.join(os.path.dirname(file_path), "Gamesave_Backup", os.path.basename(file_path))
        if os.path.exists(backup_dir):
            response = self.userInterface.askQuestion("Backup Exists",
                                            f"A backup for file '{os.path.basename(file_path)}' already exists. \n Do you want to overwrite it?",
                                            icon='warning')
            if response == 'yes':
        ...
        self.userInterface.create_backup_dir(file_path)
